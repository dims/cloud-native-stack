...
func TestBuildRecipeMergesMatchingOverlay(t *testing.T) {
...
`))
	...
	rec, err := NewBuilder().BuildFromQuery(context.TODO(), query)
	if err != nil {
...
func TestBuildRecipeIgnoresNonMatchingOverlay(t *testing.T) {
	t.Cleanup(setRecommendationData(t, `base:
  - type: K8s
    subtypes:
      - subtype: control-plane
...
func TestBuildRecipePreservesContext(t *testing.T) {
	t.Cleanup(setRecommendationData(t, `base:
  - type: SystemD
    subtypes:
