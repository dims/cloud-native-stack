...
func TestBundler_Make(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "valid recipe with gpu-operator component",
			recipe:  createTestRecipeResult(),
			wantErr: false,
			verifyFunc: func(t *testing.T, outputDir string) {
				bundleDir := filepath.Join(outputDir, "gpu-operator")
		...
		{
			name:    "missing gpu-operator component",
...
// Helper function to create a RecipeResult without gpu-operator
